                    ...
                    <v-stepper
                      ...
                      >
                        ...
                        <v-stepper-content step="1">
                          ...
                          >Create the Base user Account that stores your contracts' adresses. Basically a UserManager</v-card>
                        ...
                        <v-stepper-content step="2">
                          ...
                          >Create the Token manager that stores your tokens' adresses, name, color, and more. . . </v-card>
                          <v-btn
                            class="success"
                            v-on:click="createTokenManager"
                          >
                            Continue
                          </v-btn>
                        ...
                        <v-stepper-content step="3">
                          ...
                          >Create the Budget manager that stores your Tx's that contain categories and decriptions.</v-card>
                          <v-btn
                            class="success"
                            v-on:click="createBudgetManager"
                          >
                            Continue
                          </v-btn>
                        ...
                        <v-stepper-content step="4">
                          ...
                          >Create the Wallet Manager that keeps a record of which wallets you own, and makes sure you own them.</v-card>
                          <v-btn
                            class="success"
                            v-on:click="createWalletManager"
                          >
                            Continue
                          </v-btn>
                        ...
                        <v-stepper-content step="5">
                          ...
                          >Congratulations, your contracts are now ready ! you may now sign-in</v-card>
                          <OneWalletLogin/>
                          <v-btn text>
                            Cancel
                          </v-btn>
...
export default {
  ...
  methods: {
    ...
    createUser: async function() {
      ...
      const data = params
      console.log(data)
      const unattachedContract = await this.initializeContract(userFactoryContract, store.state.OPENFI);
    ...
    createTokenManager: async function(){
      ...
      const unattachedUserFactoryContract = await this.initializeContract(userFactoryContract, store.state.OPENFI);
      const userFactoryContract1 = wallet.attachToContract(unattachedUserFactoryContract);
      const manageraddr = await userFactoryContract1.methods.getUserContractAddress(zeroxAddr).call();
      ...
      const unattachedUserManagerContract = await this.initializeContract(userManagerContract, manageraddr);
      const contract = wallet.attachToContract(unattachedUserManagerContract);
      const gas = await contract.methods.createTokenContract(data).estimateGas(options1);
      console.log(gas)

      const value = await contract.methods.createTokenContract(data).send(options)
      .then(this.e6 += 1)
      console.log(value)
    },
      ...
      const unattachedUserManagerContract = await this.initializeContract(userManagerContract, manageraddr);
      const contract = wallet.attachToContract(unattachedUserManagerContract);
      const gas = await contract.methods.createWalletContract(data).estimateGas(options1);
      console.log(gas)

      const value = await contract.methods.createWalletContract(data).send(options)
      .then(this.e6 += 1)
      console.log(value)
    },
      ...
      const unattachedUserManagerContract = await this.initializeContract(userManagerContract, manageraddr);
      const contract = wallet.attachToContract(unattachedUserManagerContract);
      const gas = await contract.methods.createBudgetContract(data).estimateGas(options1);
      console.log(gas)

      const value = await contract.methods.createBudgetContract(data).send(options)
      .then(this.e6 += 1)
      console.log(value)
    },
