...
export default {
  ...
  methods: {
    ...
    getUserContracts: async function(wallet) {
      ...
      const unattachedContract = await this.initializeContract(contractUserFactory, store.state.OPENFI);
      const contract = wallet.attachToContract(unattachedContract);
      const UserManagerAddr = await contract.methods.getUserContractAddress(zeroxAddr).call();
      console.log("UserManager address : " + UserManagerAddr);
      store.commit("setUserManagerAddr", UserManagerAddr);


      const unattachedManagerContract = await this.initializeContract(contractUserManager, UserManagerAddr);
      const UserManager = wallet.attachToContract(unattachedManagerContract);
      const UserWalletAddr = await UserManager.methods.getUserWalletContractAddress().call()
      ...
      const UserTokenAddr = await UserManager.methods.getUserTokenContractAddress().call();
      console.log("UserTokenAddr : " + UserTokenAddr);
      store.commit("setUserTokenAddr", UserTokenAddr);

      const UserBudgetAddr = await UserManager.methods.getUserBudgetContractAddress().call();
